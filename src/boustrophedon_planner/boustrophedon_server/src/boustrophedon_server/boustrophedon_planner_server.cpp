...
BoustrophedonPlannerServer::BoustrophedonPlannerServer() :
    private_node_handle_("~"), action_server_(node_handle_,
                                              "plan_path",
                                              boost::bind(&BoustrophedonPlannerServer::executePlanPathAction, this, _1),
                                              false),
    conversion_server_{node_handle_.advertiseService("convert_striping_plan_to_path",
                                                     &BoustrophedonPlannerServer::convertStripingPlanToPath,
                                                     this)} {
  std::size_t error = 0;
  ...
  error += static_cast<std::size_t>(!rosparam_shortcuts::get("plan_path",
                                                             private_node_handle_,
                                                             "enable_stripe_angle_orientation",
                                                             enable_orientation_));
  error += static_cast<std::size_t>(
      !rosparam_shortcuts::get("plan_path", private_node_handle_, "travel_along_boundary", travel_along_boundary_));
  error += static_cast<std::size_t>(!rosparam_shortcuts::get("plan_path",
                                                             private_node_handle_,
                                                             "allow_points_outside_boundary",
                                                             allow_points_outside_boundary_));
  error += static_cast<std::size_t>(
  ...
  striping_planner_.setParameters({stripe_separation_,
                                   ...
                                   travel_along_boundary_,
                                   enable_half_y_turns_,
                                   points_per_turn_,
                                   turn_start_offset_});
  ...
  if (publish_polygons_) {
    initial_polygonPublisher_ = private_node_handle_.advertise<geometry_msgs::PolygonStamped>("initial_polygon", 1);
    preprocessed_polygonPublisher_ =
...
void BoustrophedonPlannerServer::executePlanPathAction(const boustrophedon_msgs::PlanMowingPathGoalConstPtr& goal) {
  ...
  if (!checkPolygonIsValid(polygon)) {
    action_server_.setAborted(Server::Result(),
                              std::string("Boustrophedon planner does not work for polygons of "
                                          "size "
                                          "< 3."));
    return;
  }
  if (!polygon.is_simple()) {
    action_server_.setAborted(Server::Result(),
                              std::string("Boustrophedon planner only works for simple (non "
  ...
  if (publish_polygons_) {
    initial_polygonPublisher_.publish(goal->property);
    preprocessed_polygonPublisher_.publish(convertCGALPolygonToMsg(polygon));
  }

  Polygon fill_polygon;
  if (!outline_planner_.addToPath(polygon, robot_position, path, fill_polygon)) {
    action_server_.setAborted(Server::Result(),
                              std::string("Boustrophedon planner failed because "
                                          "outline_layer_count "
...
bool BoustrophedonPlannerServer::convertStripingPlanToPath(boustrophedon_msgs::ConvertPlanToPath::Request& request,
  ...
  std::transform(request.plan.points.begin(),
                 request.plan.points.end(),
...
double BoustrophedonPlannerServer::getStripeAngleFromOrientation(const geometry_msgs::PoseStamped& robot_position) {
  tf::Quaternion quat(robot_position.pose.orientation.x,
                      robot_position.pose.orientation.y,
                      robot_position.pose.orientation.z,
                      robot_position.pose.orientation.w);
  tf::Matrix3x3 m(quat);
