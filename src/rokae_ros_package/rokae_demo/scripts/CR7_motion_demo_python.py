...
class MoveGroupPythonInterfaceTutorial(object):
    ...
    def go_to_home_position(self):
        ...
        move_group.set_joint_value_target([0, pi/6, -pi/2, 0, -pi/3, 0])
        move_group.go()
        rospy.sleep(15)
...
def main():
    try:
        ...
        tutorial.add_box("box1", 0.25, 0.20, 0.5, 0.325, 0.40, 0.27)
        tutorial.add_box("box2", 0.05, 0.05, 0.05, 0.7, -0.2, 0.32)

        # Move Rokae arm to a pose goal with obstacle avoidance
        tutorial.go_to_pose_goal(0.7, -0.2, 0.35)
        # tutorial.wait_for_movement_finished()
        tutorial.attach_box()

        # Move Rokae arm to a joint state goal with obstacle avoidance
        tutorial.go_to_joint_state(100 * pi / 180)
        # tutorial.wait_for_movement_finished()
