...
def generate_init_pose() -> Pose:
    ...
    print(
        f"Quaternion validity : {is_valid_quaternion(init_pose.orientation)}")
...
def generate_waypoints(init_pose: Pose, target_offset: list) -> list:
    ...
    for target in target_offset:
        for k, v in target.items():
            new_pose.position.x = new_pose.position.x + \
                v if k == 'x' else new_pose.position.x
            new_pose.position.y = new_pose.position.y + \
                v if k == 'y' else new_pose.position.y
            new_pose.position.z = new_pose.position.z + \
                v if k == 'z' else new_pose.position.z
