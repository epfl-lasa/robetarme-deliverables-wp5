...
class PointCloudTransformer:
    def __init__(self):
        ...
        self.color_image_with_roi = None
        self.color_image_roi_pub = None  
    ...
    def color_image_callback(self, msg):
        ...
        color_image_with_roi_msg = msg
        color_image_with_roi_msg.header.frame_id = "base_link"
        self.color_image_roi_pub.publish(color_image_with_roi_msg)
    ...
    def main(self):
        ...
        rate = rospy.Rate(3)
        self.color_image_roi_pub = rospy.Publisher('/camera/color_image_with_roi', Image, queue_size=10)
        ...
        publisher_roi = rospy.Publisher("/camera/color/image_raw_frame", Image, queue_size=10)
        ...
        while not rospy.is_shutdown():
            if self.roi_mask is not None:
                self.color_image_with_roi = cv2.addWeighted(self.color_image, 0.7, cv2.cvtColor(self.roi_mask, cv2.COLOR_GRAY2BGR), 0.3, 0)
                cv2.imshow("Color Image", self.color_image_with_roi)
                self.save_color_image(self.color_image_with_roi)
