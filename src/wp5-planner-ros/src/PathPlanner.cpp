...
void PathPlanner::setTarget(Eigen::Quaterniond target_quat,
...
Eigen::Vector3d PathPlanner::findCenter(const std::vector<Eigen::Vector3d>& vertices) {
  ...
  for (const auto& vertex : vertices) { center += vertex; }
...
void PathPlanner::scalePolygon(std::vector<Eigen::Vector3d>& vertices) {
  ...
  for (auto& vertex : vertices) { vertex *= scaleFactor; }
...
std::vector<Eigen::Vector3d> PathPlanner::get_planner_points() {
  ...
  Eigen::Affine3d transformation =
      Eigen::Translation3d(targetPos(0), targetPos(1), targetPos(2)) * targetQuat.conjugate();
...
nav_msgs::Path PathPlanner::get_transformed_path(const nav_msgs::Path& originalPath) {
  ...
  for (const auto& pose : originalPath.poses) {
    ...
    Eigen::Quaterniond originalOrientation(pose.pose.orientation.w,
...
std::vector<std::vector<double>> PathPlanner::convertPathPlanToVectorVector(const nav_msgs::Path& input_path) {
