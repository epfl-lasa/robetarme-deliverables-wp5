 ...
 * @brief Create a ROS interface with respect to the ROS version to communicate
...
RosInterfaceNoetic::RosInterfaceNoetic(ros::NodeHandle& n, string robotName)
    : nh_(n), robotName_(robotName) {
  // Try to load parameters from YAML file
  try {
    // Load parameters from YAML file
    string alternativeYamlPath =
        string(WP5_ROS_INTERFACE_DIR) + "/config/ros_interface_config.yaml";
    string yamlPath = string(WP5_ROS_INTERFACE_DIR) +
                      "/../../config/ros_interface_config.yaml";
    ...
    // ROS init
    subFTsensor_ =
        nh_.subscribe(FTTopic, 10, &RosInterfaceNoetic::FTCallback, this);
    subState_ = nh_.subscribe(actualStateTopic, 10,
                              &RosInterfaceNoetic::jointStateCallback, this);
    pubState_ =
        nh_.advertise<std_msgs::Float64MultiArray>(commandStateTopic, 1000);

    pubStateDS_ =
        nh_.advertise<std_msgs::Float64MultiArray>("desiredDsTwist", 1000);
    pubStateCartesianTwistEEF_ = nh_.advertise<std_msgs::Float64MultiArray>(
        "actualCartesianTwistEEF", 1000);
    pubStateCartesianPoseEEF_ = nh_.advertise<geometry_msgs::PoseStamped>(
        "actualCartesianPoseEEF", 1000);
...
void RosInterfaceNoetic::jointStateCallback(
  ...
  if (!msg->position.empty()) {
    ...
    if (robotName_ == "ur5_robot") {
      // swap the position to have each joint in the kinematic order, ONLY FOR
      // UR%
...
void RosInterfaceNoetic::FTCallback(
    const geometry_msgs::WrenchStamped::ConstPtr& msg) {
  if (msg->wrench.force.x != 0.0 || msg->wrench.force.y != 0.0 ||
...
RosInterfaceNoetic::receiveState() {
  ros::spinOnce();
...
void RosInterfaceNoetic::setCartesianPose(
    pair<Eigen::Quaterniond, Eigen::Vector3d> pairActualQuatPos) {
  ...
  actualPoseMsg.header.stamp =
      ros::Time::now();  // Set the current time as the timestamp
